    ...
    public class ReportController : TaskTrackerBaseController
    {
        ...
        public ActionResult ViewReleaseParameters(string id, string paramsJSON)
        {
            ...
            try
            {
                ...
                var response = client.PostAsJsonAsync(cWebApiNames.APIGetReleaseDetails, viewmodel).Result;
                var responseMsg = response.Content.ReadAsStringAsync().Result;
                if (response.IsSuccessStatusCode)
                {
                    viewmodel.lstReleaseDetailsDM = JsonConvert.DeserializeObject<IList<ReleaseDetailsDM>>(responseMsg);                    
                }
                else
                {
                    throw new Exception(response.ReasonPhrase);
