...
namespace Task_tracker_WebAPI.Areas.Reports.Controllers
{
    public class ReportsController : ApiController
    {
        ...
        [HttpPost]
        public IList<ReleaseDetailsDM> GetReleaseDetails(ReleaseDM relDM)
        {
            List<ReleaseDetailsDM> lst = null;
            try
            {
                using (var uow = new UnitOfWork())
                {
                    lst = uow.ReleaseInstRepo.getReleaseDetailsList(relDM.release_syscode);
                }
            }
            catch (Exception ex)
            {
                Log.LogError(ex.Message, "", relDM.logged_in_user.ToString(), "GetReleaseDetails", "ReportAPI");
            }
            return lst;
        }
