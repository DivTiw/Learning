...
namespace Task_Tracker_Library.Repository
{
    public class ReleaseRepository : TTBaseRepository<ReleaseInstructions>
    {
        ...
        public List<ReleaseDetailsDM> getReleaseDetailsList(int release_syscode)
        {
            List<ReleaseDetailsDM> lst = new List<ReleaseDetailsDM>();

            var query = context.ReleaseDetails
                        .Where(r => r.release_syscode == release_syscode && r.is_active && !r.is_deleted)
                        .Join(context.ProjectParameterMaster, rd => rd.parameter_syscode, pp => pp.parameter_syscode
                           , (rd, pp) => new ReleaseDetailsDM
                            {
                               ParameterMaster = pp,
