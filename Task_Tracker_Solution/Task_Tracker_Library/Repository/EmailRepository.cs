...
namespace Task_Tracker_Library.Repository
{
    public class EmailRepository
    {
        ...
        private static int project_syscode;

        public EmailRepository(CEDBContext _context)
        {
            ceContext = _context;
            project_syscode = Convert.ToInt32(ConfigurationManager.AppSettings["TokenProjectSysCode"]);
        ...
        public bool SendEmail(int? userSyscode, int template_syscode, string subject, string displayName, string fromID, string body, IEnumerable<int> toEmployeeID, IEnumerable<int> ccEmployeeID = null, IEnumerable<int> bccEmployeeID = null)
        {
            ...
            //Log.LogDebug("After fetching email ID's of the individuals", "", "", "Send Email", "EmailRepo");
            ...
            if (toEmailID.Count() > 0)
            {
                ...
                string result = ceContext.proc_save_email_to_outbox(userSyscode, template_syscode, displayName, fromID, strToEmails, strCCEmails, subject, body, userSyscode, DateTime.Now, 0, "0", project_syscode, null, null);

                //Log.LogDebug($"Result of save email procedure call {result}", "", "", "Send Email", "EmailRepo");
        ...
        //public bool SendEmail(IEnumerable<int> toEmployeeID, int templateId, string subject, string displayName = "", string body = "", IEnumerable<int> ccEmployeeID = null, IEnumerable<int> bccEmployeeID = null)
        //{
